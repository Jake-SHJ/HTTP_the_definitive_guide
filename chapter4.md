...
1. '마음대로' 커넥션 끊기
   ...
   - HTTP 어플리케이션은 언제든지 지속 커넥션을 임의로 끊을 수 있다.
   - 지속 커넥션이 일정 시간 동안 요청을 전송하지 않고 유휴 상태에 있으면 서버는 그 커넥션을 끊을 수 있지만, 서버는 그 시점에서 클라이언트가 데이터를 전송하지 않을 것이라고 확신하지 못한다.

2. Content-Length와 Truncation

   - 각 HTTP 응답은 본문의 정확한 크기 값을 가지는 Content-Length 헤더를 가지고 있어야 한다.
   - 클라이언트나 프락시가 커넥션이 끊어졌다는 HTTP 응답을 받은 후, 실제 전달된 엔터티의 길이와 Content-Length의 값이 일치하지 않거나 Content-Length 자체가 존재하지 않으면 수신자는 데이터의 명확한 길이를 서버에게 물어봐야 한다.
...
4. 우아한 커넥션 끊기
   ...
   - 전체 끊기와 절반 끊기
     ...
     - 애플리케이션은 TCP 입력 채널과 출력 채널 중 한 갸만 끊거나 둘 다 끊을 수 있다.
     - TCP 커넥션의 입력 채널과 출력 채널의 커넥션을 모두 끊는 것을 '전체 끊기'라 하고, 하나를 개별적으로 끊으면 '절반 끊기'라 한다.
   ...
   - 우아하게 커넥션 끊기
     ...
     - 일반적으로 애플리케이션이 우아한 커넥션 끊기를 구현하는 것은 애플리케이션 자신의 출력 채널을 먼저 끊고 다른 쪽에 있는 기기의 출력이 끊기는 것을 기다리는 것이다.
     - 양쪽에서 더는 데이터를 전송하지 않을 것이라고 알려주면, 커넥션은 리셋의 위험 없이 온전히 종료된다.
     - 상대방이 절반 끊기를 구현했다는 보장도 없고 절반 끊기를 했는지 검사해준다는 보장도 없기 때문에, 커넥션을 우아하게 끊고자 하는 애플리케이션은 출력 채널에 절반 끊기를 하고 난 후에도 데이터나 스트림의 끝을 식별하기 위해 입력 채널에 대해 상태 검사를 주기적으로 해야 한다.
